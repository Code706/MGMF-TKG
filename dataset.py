...
class Dataset:
    def __init__(self, 
        ...
        self.ent2id = {}
        self.rel2id = {}
        self.data = {"train.txt": self.readFile(self.ds_path + "train.txt"),
                     "valid.txt": self.readFile(self.ds_path + "valid.txt"),
                     "test.txt":  self.readFile(self.ds_path + "test.txt")}
        self.start_batch = 0
        self.all_facts_as_tuples = None
        
        self.convertTimes()

        self.all_facts_as_tuples = set([tuple(d) for d in self.data["train.txt"] + self.data["valid.txt"] + self.data["test.txt"]])
        for spl in ["train.txt", "valid.txt", "test.txt"]:
    ...
    def convertTimes(self):
        for split in ["train.txt", "valid.txt", "test.txt"]:#
            for i, fact in enumerate(self.data[split]):
                ...
                if(len(self.data[split][i])==6):
                    self.data[split][i].append(6+100)
                    self.data[split][i].append(24+100)

                elif (len(self.data[split][i]) == 8):
                    self.data[split][i][-2]=self.data[split][i][-2]/4+6+100
                    self.data[split][i][-1]=self.data[split][i][-1]/4 + 6+100
                else:
                    self.data[split][i][-1]=int(self.data[split][i][-1]/4+6)
                    temp=((self.data[split][i][-1] / 4+6)-int(self.data[split][i][-1]/4+6))*60
                    self.data[split][i].append(temp)

    def numEnt(self):
        temp=0
        if len(self.ent2id)>len(self.rel2id):
            temp=len(self.ent2id)
        else:
            temp=len(self.rel2id)
    ...
    def nextPosBatch(self, batch_size):
        if self.start_batch + batch_size > len(self.data["train.txt"]):
            ret_facts = self.data["train.txt"][self.start_batch : ]
            self.start_batch = 0
        else:
            ret_facts = self.data["train.txt"][self.start_batch : self.start_batch + batch_size]
    ...
    def addNegFacts2(self, bp_facts, neg_ratio):
        ...
        pos_neg_group_size = 1 + neg_ratio  # 501

        facts1 = np.repeat(np.copy(bp_facts), pos_neg_group_size, axis=0)  #512 *501
    ...
    def nextBatch(self, batch_size, neg_ratio=1):
        ...
        batch = shredFacts(self.addNegFacts2(bp_facts, neg_ratio)) # bp_facts = 520 *6
