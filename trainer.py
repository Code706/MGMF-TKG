    ...
    def train(self, early_stop=False):
        ...
        for epoch in range(1, self.params.ne + 1):
            ...
            while not last_batch:
                ...
                heads, rels, tails, years, months, days, mm, mmend= self.dataset.nextBatch(self.params.bsize, neg_ratio=self.params.neg_ratio)

                last_batch = self.dataset.wasLastBatch()

                scores = self.model(heads, rels, tails, years, months, days,mm, mmend)  # scores=513024
                num_examples = int(heads.shape[0] / (1 + self.params.neg_ratio))# 512
